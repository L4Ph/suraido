...
Slides written with [todan](https://github.com/L4Ph/todan). You compose slides in JSX and
they render in a browser.

**This is not React.** JSX goes through todan's own runtime and becomes DOM directly. Do not
...
- **You cannot branch on the current step inside `render()`.** Reveals go through `<Step>`
  only; changing step does not re-render (todan just toggles an attribute).
...
Every todan style lives inside an `@layer`. **A plain selector here always wins** — no
...
To change the whole palette at once, swap the `import "todan/themes/<name>.css"` line
