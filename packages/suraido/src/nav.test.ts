...
import { advance, formatHash, LAST, parseHash, type Paths, type Pos } from "./nav.ts";
...
test("backward asks for the previous slide's last step without knowing which it is", () => {
  // That slide has not been drawn, so its reveals have not been counted. LAST is resolved once
  // it is on screen.
  expect(back([2, 0])).toEqual([1, LAST]);
  expect(back([1, 0])).toEqual([0, LAST]);
});
...
test("positional hashes round-trip, and a slide past the end is brought back", () => {
  expect(formatHash([1, 2], unnamed)).toBe("#1.2");
  expect(formatHash([1, 0], unnamed)).toBe("#1");
  expect(parseHash("#1.2", unnamed)).toEqual([1, 2]);
  expect(parseHash("", unnamed)).toEqual([0, 0]);
  expect(parseHash("#nope", unnamed)).toEqual([0, 0]);
});

test("a step past the end is left alone, because only the slide can say where its end is", () => {
  expect(parseHash("#9.9", unnamed)).toEqual([2, 9]);
  expect(parseHash("#1.-4", unnamed)).toEqual([1, 0]);
});
...
test("named slides win over their index, unnamed ones fall back to it", () => {
  ...
  expect(parseHash("#body.2", named)).toEqual([1, 2]);
  expect(parseHash("#intro", named)).toEqual([0, 0]);
});
...
test("a named link survives reordering the deck", () => {
  ...
  expect(parseHash(link, after)).toEqual([1, 0]); // the index moved, but the link still lands
});
